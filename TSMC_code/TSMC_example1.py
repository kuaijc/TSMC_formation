...
if __name__ == '__main__':
    ...
    # parameter selection
    alpha = 0.3    # the tunable step-size of DC-ADMM
    rho = 0.1  # the tunable penalty force of DC-ADMM
    beta = 0.03     # the tunable step-size of D-OGDA
    D = 0.2     # the tunable dual variable bound of D-OGDA

    matching_times = 5000   # match-iteration times
    location_times = 500    # location-iteration times
    ...
    # give the desired formation shape as two circle
    sq_3 = math.sqrt(3)
    relation_m = np.array([[8, 0], [4 * sq_3, 4], [4, 4 * sq_3], [0, 8], [-4, 4 * sq_3], [-4 * sq_3, 4],
                           [-8, 0], [-4 * sq_3, -4], [-4, -4 * sq_3], [0, -8], [4, -4 * sq_3], [4 * sq_3, -4],
                           [4, 0], [2 * sq_3, 2], [2, 2 * sq_3], [0, 4], [-2, 2 * sq_3], [-2 * sq_3, 2], [-4, 0],
                           [-2 * sq_3, -2], [-2, -2 * sq_3], [0, -4], [2, -2 * sq_3], [2 * sq_3, -2]])
    ...
    plt.ylabel(r'$\frac{\Vert d_{0}-d_{0}^{*}\Vert_{2}}{\Vert d_{0}^{*}\Vert_{2}}$',
               fontsize=20, fontname='Times New Roman')
    plt.xticks(np.linspace(0, location_times, 3), fontsize=16)
    plt.yticks(fontsize=16)
    plt.xlim(0, location_times)
    plt.legend(prop={'family': 'Times New Roman', 'size': 15}, loc='upper right', bbox_to_anchor=(1.0, 1.0))
